...
class Solution1 {
    public List<List<String>> partition(String s) {
        ...
        getAllPalindromePartition(results,s, ans);
        return results;
    }

    private void getAllPalindromePartition(List<List<String>> results, String s, List<String> ans) {
        int n = s.length();
        if(n==0){
        ...
        for (int i = 0; i < n; i++) {
            ...
            if(isPalindrome(prefix)){
                ans.add(prefix);
                getAllPalindromePartition(results,s.substring(i+1,n),ans); //right partition getting called from.
